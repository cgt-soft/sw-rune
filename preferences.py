...
complex_classification = {  'classes': { 'TNK' : 'Tank', 'PDD' : 'Pure Damage Dealer', 'VDD' : 'Violent Damage Dealer',
...
simple_classification = {   'classes': { 'SUP': 'Support', 'ATK': 'Attack'},
                            'monster_types': {'SUP': {'SETS': ['Energy', 'Swift', 'Despair', 'Violent', 'Focus',
                                                               'Guard', 'Endure', 'Shield', 'Revenge', 'Will',
                                                               'Nemesis', 'Fight', 'Determination', 'Enhance',
                                                               'Accuracy', 'Tolerance'],
                                                        'SUBS': ['DEF', 'HP', 'SPD', 'ACC', 'RES']},
                                              'ATK': {'SETS': ['Swift', 'Blade', 'Fatal', 'Despair', 'Violent',
                                                               'Revenge', 'Rage', 'Will', 'Nemesis', 'Vampire',
                                                               'Destroy', 'Fight', 'Determination', 'Enhance',
                                                               'Accuracy', 'Tolerance'],
                                                      'SUBS': ['SPD', 'ACC', 'RES', 'ATK', 'CD', 'CR']}}}
default_settings = {'rune_sets': ['Energy', 'Guard', 'Swift', 'Blade', 'Rage', 'Focus', 'Endure', 'Fatal', 'Despair',
                    ...
                    'substat_weights': {'HP': 1.0, 'DEF': 1.0, 'SPD': 1.2, 'CR': 1.8, 'CD': 1.8, 'ATK': 1.0,
                                        'RES': 0.6, 'ACC': 0.6},
                    'Complex': complex_classification,
                    'Simple': simple_classification,
                    'Custom': {'classes': {}, 'monster_types': {}},
...
class PreferencesDialog(QtWidgets.QDialog, Ui_preferencesDialog):
    ...
    def set_connections(self):
        ...
        self.classificationComboBox.currentTextChanged.connect(self.classification_combobox_changed)

    def classification_combobox_changed(self):
        self.settings['classification'] = self.classificationComboBox.currentText()
        if self.settings['classification'] == 'Custom':
            self.textEdit.setReadOnly(False)
            classification = 'Complex'
        else:
            classification = self.settings['classification']
        txt = json.dumps(self.settings[classification]['monster_types'], sort_keys=True, indent=4)
        self.textEdit.clear()
    ...
    def restore_settings(self):
        ...
        for rune_set in self.settings['rune_sets']:
            ...
            self.setslistWidget.addItem(item)
        # Set json monster types (Advanzed Tab)
        self.classificationComboBox.setCurrentText(self.settings['classification'])
        txt = json.dumps(self.settings[self.settings['classification']]['monster_types'], sort_keys=True, indent=4)
        self.textEdit.clear()
        self.textEdit.insertPlainText(txt)
    ...
    def apply_button_clicked(self):
        ...
        self.settings[self.settings['classification']]['monster_types'] = obj
